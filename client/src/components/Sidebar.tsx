...
const Sidebar = ({
  ...
  const { logout, user } = {
    ...
    user: { name: "naved bagwan", email: "bagwannaved96@gmail.com" },
  };
  ...
  const navItems = [
    ...
    { name: "Scheduler", icon: CalendarDaysIcon, path: "/schduler" },
    { name: "AI Composer", icon: Wand2Icon, path: "/ai-composer" },
  ];
  return (
    <div
      className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col h-full transform  transtion-transform duration-200 ease-in=-out md:relative md:translate-x-0 ${isOpen ? "transaltex-0" : "-transalate-x-full"}`}
    >
        ...
        {navItems.map((item) => {
          ...
          return (
            <NavLink
              ...
              className={` flex items-center gap-3 px-3 py-2.5 rounded text-sm transition-all duration-150 border ${isActive ? "bg-red-50 text-red-600 border-red-100" : "text-slave-500 hover:bg-slate-50 border-transparent hover:text-slate-7000"}`}
            >
              <item.icon
...
/*
      <div className=" p=-4 border-t border-slate-100">
        <div className=" flex items-center gap-3 p-2 rounded-xl hover-bg-slate-50 transtion-colors">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm text-slare-800 truncate">{user.name}</div>
            <div className="text text-slate0400 truncate">{user.email}</div>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-t border-slate-100">
        ...
        <button
          onClick={logout}
          className="mt-1 flex items-center gap-2 px-3 py-2 w-full rounded text-sm text-slate-500 hover:bg-red-5 hover:text-red-500 transition-all-duration-150"
        >
